...
import {
  Users, Plus, Milk, IndianRupee, CalendarCheck, Phone, MapPin, Loader2,
  TrendingUp, Wallet, AlertCircle, CheckCircle2, Pencil, ClipboardList, Trash2,
} from 'lucide-react';
...
import type { RegularCustomer, DailyOrderRecord } from '../backend';
...
function AddCustomerForm({ onClose }: AddCustomerFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const qty = parseFloat(form.dailyMilkQuantity);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, dailyMilkQuantity: e.target.value })}
            required
          />
        </div>
        <div className="space-y-1.5">
      ...
      <DialogFooter className="pt-2">
        <Button type="button" variant="outline" onClick={onClose} disabled={addCustomer.isPending}>
          Cancel
        </Button>
...
function EditCustomerForm({ customer, onClose }: EditCustomerFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <DialogFooter className="pt-2">
        <Button type="button" variant="outline" onClick={onClose} disabled={updateCustomer.isPending}>
          Cancel
        </Button>
...
function DailyOrdersDialog({ customer, open, onOpenChange }: DailyOrdersDialogProps) {
  const { data: records, isLoading, error } = useDailyOrderRecordsByCustomer(
  ...
  const [recordToDelete, setRecordToDelete] = useState<DailyOrderRecord | null>(null);
  const [formError, setFormError] = useState('');
  const [form, setForm] = useState({
    date: today,
    quantityDelivered: customer.dailyMilkQuantity.toString(),
    amountCharged: (customer.dailyMilkQuantity * customer.pricePerLitre).toFixed(2),
    notes: '',
  });

  const handleQtyChange = (val: string) => {
    const qty = parseFloat(val);
    const autoAmount = isNaN(qty) ? '' : (qty * customer.pricePerLitre).toFixed(2);
    setForm((f) => ({ ...f, quantityDelivered: val, amountCharged: autoAmount }));
  };

  const handleAddSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormError('');
    const qty = parseFloat(form.quantityDelivered);
    const amt = parseFloat(form.amountCharged);
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex-1 overflow-hidden flex flex-col gap-4 min-h-0">
            ...
            {sortedRecords.length > 0 && (
              <div className="flex gap-3 flex-wrap">
                <div className="bg-secondary/60 rounded-lg px-3 py-2 text-sm">
                  ...
                  <span className="font-semibold text-foreground">{sortedRecords.length}</span>
                </div>
                <div className="bg-secondary/60 rounded-lg px-3 py-2 text-sm">
                  <span className="text-muted-foreground">Total Qty: </span>
                  <span className="font-semibold text-foreground">{totalQty.toFixed(1)}L</span>
                </div>
                <div className="bg-secondary/60 rounded-lg px-3 py-2 text-sm">
                  <span className="text-muted-foreground">Total Amount: </span>
                  <span className="font-semibold text-foreground">₹{totalAmt.toFixed(2)}</span>
                </div>
              </div>
            )}
            ...
            <ScrollArea className="flex-1 border border-border rounded-lg min-h-0">
              {isLoading ? (
                ...
                <div className="p-8 text-center text-muted-foreground text-sm">
                  ...
                  <p className="mt-1">Click "Add Daily Record" to get started.</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {sortedRecords.map((record) => (
                      <TableRow key={Number(record.recordId)}>
                        ...
                        <TableCell className="text-right text-sm">{record.quantityDelivered.toFixed(1)}</TableCell>
                        <TableCell className="text-right text-sm font-semibold">₹{record.amountCharged.toFixed(2)}</TableCell>
            ...
            {/* Add record form */}
            {showAddForm ? (
              <form onSubmit={handleAddSubmit} className="border border-border rounded-lg p-4 space-y-3 bg-secondary/20">
                <p className="text-sm font-semibold text-foreground">Add Daily Record</p>
                {formError && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    <Label htmlFor="rec-qty" className="text-xs">Quantity (L) *</Label>
                    <Input
                      id="rec-qty"
                      type="number"
                      step="0.1"
                  ...
                  <div className="space-y-1">
                    <Label htmlFor="rec-amt" className="text-xs">Amount (₹) *</Label>
                    <Input
                      id="rec-amt"
                      type="number"
                      step="0.01"
                ...
                <div className="space-y-1">
                  <Label htmlFor="rec-notes" className="text-xs">Notes (optional)</Label>
                  <Textarea
                    id="rec-notes"
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <Button type="submit" size="sm" disabled={addRecord.isPending}>
                    {addRecord.isPending ? (
                      ...
                      <><Plus className="h-3.5 w-3.5 mr-1.5" />Save Record</>
                    )}
                  </Button>
                </div>
              </form>
      ...
      {/* Delete confirmation */}
      <AlertDialog open={!!recordToDelete} onOpenChange={(o) => { if (!o) setRecordToDelete(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Record?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete the record for{' '}
              <strong>{recordToDelete?.date}</strong> ({recordToDelete?.quantityDelivered.toFixed(1)}L — ₹{recordToDelete?.amountCharged.toFixed(2)}).
...
function CustomerCard({ customer }: CustomerCardProps) {
  ...
  return (
    <>
      <Card className={`relative overflow-hidden transition-shadow hover:shadow-md ${!customer.isActive ? 'opacity-60' : ''}`}>
        {/* Status badge */}
        ...
        <CardHeader className="pb-3 pr-20">
          <CardTitle className="text-lg font-bold text-foreground leading-tight">{customer.name}</CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-secondary/60 rounded-lg p-2.5 text-center">
              ...
              <p className="font-bold text-foreground text-sm">{customer.dailyMilkQuantity}L</p>
            </div>
            <div className="bg-secondary/60 rounded-lg p-2.5 text-center">
              <IndianRupee className="h-4 w-4 mx-auto mb-1 text-primary" />
              <p className="text-xs text-muted-foreground">Per Litre</p>
              <p className="font-bold text-foreground text-sm">₹{customer.pricePerLitre}</p>
            </div>
            <div className="bg-secondary/60 rounded-lg p-2.5 text-center">
              <TrendingUp className="h-4 w-4 mx-auto mb-1 text-primary" />
              <p className="text-xs text-muted-foreground">Daily Amt</p>
              <p className="font-bold text-foreground text-sm">₹{dailyAmount.toFixed(0)}</p>
            </div>
          </div>
          ...
          <div className="border border-border rounded-lg overflow-hidden">
            <div className="grid grid-cols-2 divide-x divide-border">
              <div className="p-2.5 text-center">
                ...
                <p className="font-semibold text-foreground text-sm">₹{customer.totalAmountDue.toFixed(2)}</p>
              </div>
              <div className="p-2.5 text-center">
                <p className="text-xs text-muted-foreground mb-0.5">Received</p>
                <p className="font-semibold text-success text-sm">₹{customer.amountReceived.toFixed(2)}</p>
              </div>
            </div>
            <div className={`p-2.5 text-center border-t border-border ${balance > 0 ? 'bg-destructive/10' : 'bg-success/10'}`}>
              <p className="text-xs text-muted-foreground mb-0.5">Balance Due</p>
              <p className={`font-bold text-base ${balance > 0 ? 'text-destructive' : 'text-success'}`}>
                ₹{balance.toFixed(2)}
              </p>
            </div>
          </div>
            ...
            {customers && customers.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                ...
                <Card className="col-span-2 sm:col-span-1">
                  <CardContent className="pt-4 pb-4 text-center">
                    <p className={`text-2xl font-bold ${totalBalance > 0 ? 'text-destructive' : 'text-success'}`}>
                      ₹{totalBalance.toFixed(0)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">Total Balance Due</p>
                  </CardContent>
                </Card>
              </div>
            )}
            ...
            {!isLoading && !error && filtered.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filtered.map((customer) => (
                  <CustomerCard key={Number(customer.customerId)} customer={customer} />
                ))}
              </div>
            )}
          </AdminGuard>
